...
bazel run //tools/format:cpp -- -i $PWD/main.cc

# Format all C++ files in the repository in-place
bazel run //tools/format:cpp -- -i  $(find $PWD -type f -name '*.cc' -or -name  '*.h')

# Run formatting checks (useful for CI)
bazel run //tools/format:cpp -- --dry-run --Werror $PWD/main.cc
```


## How to list the C++ souce code with clang-tidy

NOTE: Work in progress. clang tidy does not work as of now.
The paths for files in compilation database do not match what bazel expects.

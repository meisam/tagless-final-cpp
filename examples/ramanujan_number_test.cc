...
struct TaxiCabNumberProgram1 {
  auto run() -> std::optional<A> {
    ...
    // 1729 = 1^3 + 12^3 = 9^3 + 10^3
    return p.add(
      p.mul(p.mul(p.literal(1), p.literal(1)), p.literal(1)), 
...
struct TaxiCabNumberProgram2 {
  auto run() -> std::optional<A> {
    ...
    // 1729 = 1^3 + 12^3 = 9^3 + 10^3
    return p.add(
      p.mul(p.mul(p.literal(9), p.literal(9)), p.literal(9)),
...
TEST(RamanujanNumberTest, TaxiCabNumberProgram1_should_be_valid) {
  static_assert(program::Runnable<TaxiCabNumberProgram1<BasicInterpreter, int>, int>
    && "TaxiCabNumberProgram1 should be a valid program");
}

TEST(RamanujanNumberTest, TaxiCabNumberProgram2_should_be_valid) {
  static_assert(program::Runnable<TaxiCabNumberProgram2<BasicInterpreter, int>, int>
